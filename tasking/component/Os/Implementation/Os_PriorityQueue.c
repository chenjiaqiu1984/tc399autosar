...
)
{
  ...
  (void)Os_PriorityQueueHeapUp(PriorityQueue, (PriorityQueue->Dyn->QueueUsage - (Os_PriorityQueueNodeIdxType) 1u ));    /* SBSW_OS_FC_PRECONDITION */
}
...
)
{
  ...
  for(idx = OS_PRIORITYQUEUE_ROOT_IDX; idx < (Os_PriorityQueueNodeIdx_IteratorType) PriorityQueue->Dyn->QueueUsage;     /* COV_OS_INVSTATE */
      idx++)
  {
  ...
  if (OS_UNLIKELY(idx == (Os_PriorityQueueNodeIdx_IteratorType) PriorityQueue->Dyn->QueueUsage))                        /* COV_OS_INVSTATE */
  {
  ...
  else
  {
    ...
    if (OS_LIKELY(idx < (Os_PriorityQueueNodeIdx_IteratorType) PriorityQueue->Dyn->QueueUsage))
    {
